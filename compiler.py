...
def build_ast(tokens):
    ...
    def check(tokens, f):
        if not tokens:
            ...
            return True
        if not f(tokens[0]):
            ast_errors.append('Unexpected token '+('"'+tokens[0]['v'] +'" ' if 'v' in tokens[0] else '')+'at '+str(tokens[0]['pos']))
            return True
        return False
    ...
    def parse_formula(tokens):
        ...
        while input_stack:
            ...
            if t['t'] == 'VARIBLE':
                ...
                while len(tokens_stack) and tokens_stack[-1]["t"]=="MATH" and tokens_stack[-1]["v"] in ("&&", "||", ">>", "<<", ",", "+", "-", "%", "*", "!=", "==", "<", ">") and cmp_op(tokens_stack[-1]["v"],t['v']):
                ...
                while tokens_stack and tokens_stack[-1]["t"]=="MATH" and tokens_stack[-1]["v"] in ("&&", "||", ">>", "<<", ",", "+", "-", "%", "*", "!=", "==", "<", ">"):
                    if move(tokens_stack, output_tokens):
                        break
                ...
                while len(tokens_stack) and tokens_stack[-1]["t"]=="MATH" and tokens_stack[-1]["v"] in ("&&", "||", ">>", "<<", ",", "+", "-", "%", "*", "!=", "==", "<", ">"):
                ...
                while tokens_stack and tokens_stack[-1]["t"]=="MATH" and tokens_stack[-1]["v"] in ("&&", "||", ">>", "<<", ",", "+", "-", "%", "*", "!=", "==", "<", ">"):
                    if move(tokens_stack, output_tokens):
                        break
                if len(tokens_stack)<2 or not (tokens_stack[-1]["t"]=="MATH" and tokens_stack[-1]["v"]=="(" and tokens_stack[-2]["t"]=="VARIBLE"):
                    ast_errors.append('Unexpected token '+('"'+t['v'] +'" ' if 'v' in t else '')+'at '+str(t['pos']))
                    break
                tokens_stack[-2]["v"]+='-'
                if move(input_stack, remove_stack):
        ...
        if tokens_stack:
            ast_errors.append('Unexpected token '+('"'+tokens_stack[-1]['v'] +'" ' if 'v' in tokens_stack[-1] else '')+'at '+str(tokens_stack[-1]['pos']))
        return node
    ...
    def parse_instr(tokens):
        ...
        if current_token['t'] == 'BRANCHING': # <instr> ::= <branching>
            ...
            if tokens[0]['v'] == '=': # <instr> ::= <type> <name>=<formula>
                ...
                if ch is None:
                    return None
                return ASTNode({"type": var_type, "name": var_name},'DECLARAT_VARIBLE', current_token['pos'], ch.children)
                ...
                if check(tokens, lambda x: x['t'] == 'MATH' and x['v'] == ']'):
                    return None
                tokens.pop(0)
                return ASTNode({"type": 'A' + var_type, "name": var_name},'DECLARAT_ARRAY', current_token['pos'], ch.children)
                ...
                return ASTNode({"type": var_type, "name": var_name, "args": ch},'DECLARAT_FUNCTION', current_token['pos'], body)
...
def compileAST(AST):
    ...
    def add_error(node,text=''):
        compile_errors.append('Unexpected token "'+str(node.value)+'" at '+str(node.pos)+(' with message: '+text if text!='' else ''))
        return None
    ...
    def compile_branch(node, root, scope):
        ...
        for i in node.children:
            if i.type in ('DECLARAT_VARIBLE','DECLARAT_ARRAY'):
                ...
                asm.append('MOV [%v:'+i.value["name"]+':'+str(i.pos)+'%], '+(get_register_by_size(size) if i.type != 'DECLARAT_ARRAY' else 'EAX')+' # :=')
                ...
                if i.value=='for': # for(init; condition; step){body}
                    ...
                    asm.append('MOV [%v:'+init.value["name"]+':'+str(init.pos)+'%], '+get_register_by_size(size)+' # for init')
    ...
    def compile_global_function(node): # get array ASMdata
        global_offset = 0
        asm_data = [ASMdata('', 'void', 'f', node.pos, True, {"node": node, "offset": 0, "global_offset": 0, "args": [], "constants": global_variables})]
        def calc_vars_and_funcs(node, root, i=0):
            ...
            if node.type in ('DECLARAT_VARIBLE','DECLARAT_ARRAY'):
                ...
                while root.value[offset_key]%size!=0: root.value[offset_key]+=1 # alignment
                asm_data.append(ASMdata(node.value["name"], node.value["type"], 'v', node.pos, in_global, {"offset": root.value[offset_key]}))
                root.value[offset_key]+=size
                return node
            elif node.type == 'DECLARAT_FUNCTION' and i>0:
                asm_data.append(ASMdata(node.value["name"], node.value["type"], 'f', node.pos, in_global, {"node": node, "offset": 0, "args": node.value["args"]}))
                return None
            else:
                ch_list = []
...
def linkASM(asm_datas):
    ...
    def string_to_int32_array(string):
        ...
        list.append(0)
        while (len(list)%4!=0): list.append(0)
        int32_list = []
            ...
            for key in range(len(cmd)):
                ...
                if percents_text is not None:
                    ...
                    def find_varible(name, pos, for_array = False):
                        for i in asm_datas:
                            if i.type=='v' and i.name == name and i.pos == pos:
                                if i.is_global:
                                    return (get_register_by_size(get_size_type(i.datatype,for_array)),str((ASM_info["GLOBAL_VARIBLE_ADR"] + i.value["offset"])//get_size_type(i.datatype)))
                                else:
                                    return (get_register_by_size(get_size_type(i.datatype,for_array)),'SP-'+str((stack_size*4-i.value["offset"]-4)//get_size_type(i.datatype)))
                        for i in range(len(asm_data.value["args"])):
                            if asm_data.value["args"][i][1] == name and asm_data.value["args"][i][2] == pos:
                                return (get_register_by_size(get_size_type(asm_data.value["args"][i][0],for_array)),'SP-'+str(stack_size+len(asm_data.value["args"])-i))
                    ...
                    if percents_text[0] == 'g':
                        ...
                        cmd[key] = replace_value_between_percents(cmd[key],find_varible(percents_text[1],int(percents_text[2]))[0])
                    elif percents_text[0] == 'ARv':
                        cmd[key] = replace_value_between_percents(cmd[key],find_varible(percents_text[1],int(percents_text[2]),True)[0])
                    elif percents_text[0] == 'v':
                        cmd[key] = replace_value_between_percents(cmd[key],find_varible(percents_text[1],int(percents_text[2]))[1])
                    else:
    ...
    for i in range(COMPILE_SETUP["STACK_SIZE"]): ASM.append('WORD32 0')
    ASM_info["GLOBAL_VARIBLE_ADR"] = len(ASM)*4;
    while (asm_datas[0].value["global_offset"]%4!=0): asm_datas[0].value["global_offset"]+=1
    for i in range(asm_datas[0].value["global_offset"]//4): ASM.append('WORD32 -1')
    #for i in asm_datas:
    #    if i.type=='v':
    #        if i.is_global:
    ...
    for i in asm_datas[0].value['constants']:
        if i["type"] == 'string':
            ...
            for int32 in string_to_int32_array(i["value"]): ASM.append('WORD32 '+str(int32))
        else:
    ...
    for i in asm_datas:
        if i.type=='f':
            while (asm_datas[0].value["offset"]%4!=0): asm_datas[0].value["offset"]+=1
