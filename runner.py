...
class MemoryManager:
    ...
    def setmem(self, address, value):
        ...
        self.memory[address//self.mod] = self._set_word(self.memory[address//self.mod], value,address%self.mod,self.mod)
    ...
    def compile_programm(ptr, programm):
        for i in range(len(programm)):
            ...
            if cmd[0] == 'NOP':
                ...
                mm.setmem(ptr+i, 0b01000000_00000000_00000000_00000000 + (['ADC','ADD','SBC','SUB','OR','XOR','AND','MUL','SHR','SHL','CMP', 'MOD'].index(cmd[0]) << 20) + A('EAX') + F(cmd[1]))
            elif cmd[0] in ('NOT', 'NEG', 'RCL', 'RCR', 'ZEXT8', 'ZEXT16', 'EXT8', 'EXT16'):
                mm.setmem(ptr+i, 0b01010000_00000000_00000000_00000000 + (['NOT','NEG','RCL','RCR','ZEXT8','ZEXT16','EXT8','EXT16'].index(cmd[0]) << 20))
            elif cmd[0] in ('OUT', 'IN', 'MALLOC32', 'MALLOC16', 'MALLOC8'):
                mm.setmem(ptr+i, 0b01100000_00000000_00000000_00000000 + (['OUT','IN','MALLOC32','MALLOC16','MALLOC8'].index(cmd[0]) << 16))
            elif cmd[0] == 'INT' and cmd[1] in ('OUT', 'IN', 'MALLOC32', 'MALLOC16', 'MALLOC8'):
                mm.setmem(ptr+i, 0b01100000_00000000_00000000_00000000 + (['OUT','IN','MALLOC32','MALLOC16','MALLOC8'].index(cmd[1]) << 16))
            elif cmd[0] == 'CALL' and cmd[1] in ['A','E','NE','G','L','GE','LE']:
                mm.setmem(ptr+i, 0b10000000_00000000_00000000_00000000 + (['A','E','NE','G','L','GE','LE'].index(cmd[1]) << 20) + F(cmd[2]))
            elif cmd[0] == 'CALL':
                ...
                mm.setmem(ptr+i, 0b10100000_00000000_00000000_00000000 + (['A','E','NE','G','L','GE','LE'].index(cmd[1]) << 20) + F(cmd[2]))
            elif cmd[0] == 'JMP':
    ...
    def alu(self, sel_l, sel_r, alu_op):
        ...
        if 'set_flag' in alu_op:
            ...
            if alu_op['op'] in ('ADC', 'ADD', 'SBC', 'SUB','CMP'):
                ...
                if alu_op['op'] == 'ADC':
                    self.alu_flags['C'] = ((left_value&0xFFFFFFFF) + (right_value&0xFFFFFFFF) + int(self.alu_flags['C'])>0xFFFFFFFF)
                if alu_op['op'] == 'ADD':
                    self.alu_flags['C'] = ((left_value&0xFFFFFFFF) + (right_value&0xFFFFFFFF)>0xFFFFFFFF)
                if alu_op['op'] == 'SBC':
                    self.alu_flags['C'] = ((left_value&0xFFFFFFFF) + (right_value&0xFFFFFFFF) + int(self.alu_flags['C'])<0)
...
class ControlUnit:
    ...
    def execute_instruction(self, instr, data):
        ...
        if instr == 0: # NOP
            ...
            if data["RR"] != 2:
                self.tick()
                self.data_path.signal_adress_mod({0: magic_numbers.MUX_DM_MOD_4, 1: magic_numbers.MUX_DM_MOD_2}[data["RR"]])
            if data["F"] == 0: # MOV A 5
                self.tick()
                self.data_path.signal_latch_ac(magic_numbers.MUX_L_0, magic_numbers.MUX_R_DR, {"op":"ADD", "crop_right_to_int16": True})
            elif data["F"] == 1: # MOV A [5]
                self.tick(3)
                self.data_path.signal_latch_ar(magic_numbers.MUX_L_0, magic_numbers.MUX_R_DR, {"op":"ADD", "crop_right_to_int16": True})
                ...
                self.tick()
                self.data_path.signal_latch_ac(magic_numbers.MUX_L_AC, magic_numbers.MUX_R_DR, {"op":"ADD", "crop_right_to_int16": True});
            elif data["F"] == 3: # MOV A [A+5]
                self.tick(3)
                self.data_path.signal_latch_ar(magic_numbers.MUX_L_AC, magic_numbers.MUX_R_DR, {"op":"ADD", "crop_right_to_int16": True});
                self.data_path.signal_oe()
                self.data_path.signal_latch_ac(magic_numbers.MUX_L_0, magic_numbers.MUX_R_DR, {"op":"ADD"})
                ...
                self.data_path.signal_latch_ar(magic_numbers.MUX_L_AR, magic_numbers.MUX_R_DR, {"op":"ADD", "crop_right_to_int16": True})
                self.data_path.signal_oe()
                self.data_path.signal_latch_ac(magic_numbers.MUX_L_0, magic_numbers.MUX_R_DR, {"op":"ADD"})
            ...
            if data["F"] == 0: # MOV [5] A
                ...
                if data["RR"] != 2: self.data_path.signal_adress_mod(magic_numbers.MUX_DM_MOD_1)
            if data["F"] == 1: # MOV [[5]] A
                self.tick(4)
                self.data_path.signal_latch_ar(magic_numbers.MUX_L_0, magic_numbers.MUX_R_DR, {"op":"ADD", "crop_right_to_int16": True})
                self.data_path.signal_oe()
            ...
            if data["F"] == 0: # ADD 5
                self.tick()
                self.data_path.signal_latch_ac(magic_numbers.MUX_L_AC, magic_numbers.MUX_R_DR, {"op": ['ADC','ADD','SBC','SUB','OR','XOR','AND','MUL','SHR','SHL','CMP', 'MOD'][data["OPER"]], "crop_right_to_int16": True, 'set_flag': True})
            elif data["F"] == 1: # ADD [5]
                self.tick(3)
                self.data_path.signal_latch_ar(magic_numbers.MUX_L_0, magic_numbers.MUX_R_DR, {"op":"ADD", "crop_right_to_int16": True})
                self.data_path.signal_oe()
                self.data_path.signal_latch_ac(magic_numbers.MUX_L_AC, magic_numbers.MUX_R_DR, {"op":['ADC','ADD','SBC','SUB','OR','XOR','AND','MUL','SHR','SHL','CMP', 'MOD'][data["OPER"]], 'set_flag': True})
            ...
            self.tick(1)
            self.data_path.signal_latch_ac(magic_numbers.MUX_L_AC, magic_numbers.MUX_R_DR, {"un": ['NOT','NEG','RCL','RCR','ZEXT8','ZEXT16','EXT8','EXT16'][data["OPER"]], 'set_flag': True})
        elif instr == 6: # INT
            if data["INT_CODE"] == 0: # OUT
                ...
                self.tick(3)
                self.data_path.signal_malloc(magic_numbers.MUX_L_AC, magic_numbers.MUX_R_1, {"op": "MUL", "ceil_div_2": True})
                self.data_path.signal_latch_ac(magic_numbers.MUX_L_0, magic_numbers.MUX_R_DR, {"op": "ADD"})
                self.data_path.signal_latch_ac(magic_numbers.MUX_L_AC, magic_numbers.MUX_R_1, {"op": "MUL", "unceil_div_2": True})
            elif data["INT_CODE"] == 4: # MALLOC8
                self.tick(3)
                self.data_path.signal_malloc(magic_numbers.MUX_L_AC, magic_numbers.MUX_R_1, {"op": "MUL", "ceil_div_4": True})
                self.data_path.signal_latch_ac(magic_numbers.MUX_L_0, magic_numbers.MUX_R_DR, {"op": "ADD"})
                self.data_path.signal_latch_ac(magic_numbers.MUX_L_AC, magic_numbers.MUX_R_1, {"op": "MUL", "unceil_div_4": True})
            ...
            if data["COND"] == 0:
                ...
                need_jump = True
            elif data["COND"] == 3 and ((self.data_path.sign() ^ self.data_path.overflow()) or self.data_path.zero()) == 0:
                need_jump = True
                ...
                need_jump = True
            elif data["COND"] == 6 and ((self.data_path.sign() ^ self.data_path.overflow()) or self.data_path.zero()) == 1:
                need_jump = True
            ...
            if instr == 8 and need_jump:
                if data["F"] == 0: # CALL 5
                    ...
                    self.data_path.signal_latch_ip(magic_numbers.MUX_L_0, magic_numbers.MUX_R_DR, {"op":"ADD", "crop_right_to_int16": True})
                    ...
                    self.data_path.signal_latch_ip(magic_numbers.MUX_L_AR, magic_numbers.MUX_R_DR, {"op":"ADD", "crop_right_to_int16": True})
                else:
                    raise "E709"
            ...
            if data["F"] == 0: # PUSH 5
                self.tick(2)
                self.data_path.signal_latch_ar(magic_numbers.MUX_L_0, magic_numbers.MUX_R_SP, {"op":"ADD"})
                self.data_path.signal_wr(magic_numbers.MUX_L_0, magic_numbers.MUX_R_DR, {"op": "ADD", "crop_right_to_int16": True})
                ...
                self.tick(2)
                self.data_path.signal_latch_ar(magic_numbers.MUX_L_0, magic_numbers.MUX_R_SP, {"op":"ADD"})
                self.data_path.signal_wr(magic_numbers.MUX_L_AC, magic_numbers.MUX_R_DR, {"op": "ADD", "crop_right_to_int16": True})
